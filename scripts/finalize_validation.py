...
P=Path(__file__).resolve().parents[1]/'outputs/aside_browser_20260905';V=P/'05_validation'
b=json.loads((V/'browser.json').read_text());s=json.loads((V/'static.json').read_text());assert b['pass'] and s['pass']
(V/'05_validation_report.md').write_text('''# 5단계 — 파일 구조와 브라우저 검증
...
''')
files=sorted(f for f in P.rglob('*') if f.is_file() and f.relative_to(P).parts[0][:2] in ['00','01','02','03','04'])
(V/'manifest.sha256').write_text(''.join(f'{hashlib.sha256(f.read_bytes()).hexdigest()}  {f.relative_to(P)}\n' for f in files))
(V/'file_tree.txt').write_text('\n'.join(str(f.relative_to(P)) for f in sorted(P.rglob('*')) if f.is_file())+'\n')
print('Stage 5 PASS;',len(files),'stage0-4 files hashed')
